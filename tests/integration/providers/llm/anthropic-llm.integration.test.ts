...
describe.skipIf(!hasEnvVars('ANTHROPIC_API_KEY'))('Anthropic LLM Integration', () => {
	...
	it('should validate a valid API key', async () => {
		const result = await provider.validateApiKey(requireEnv('ANTHROPIC_API_KEY'));
	...
	it('should return false for an invalid API key', async () => {
		...
		try {
			...
			// Anthropic validateApiKey throws for non-401 errors, which is acceptable
		}
	});
